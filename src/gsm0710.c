...
// vitorio, only to use if necessary (don't ask in what i was thinking
// when i wrote this)
#define TRUE	1
#define FALSE	0
...
#if 0
/*
 * Opens USSP port for use. * * PARAMS: * port - port number * RETURNS *
...
)
{
    ...
    */
    ...
    if (count > 127) {
      ...
      prefix[3] = 1 | (count << 1);
		...
		/**
			* Modified as requested in last e-mail.
			*/
		postfix[0] = make_fcs (prefix + 1, prefix_length - 1);
  ...
  else				// advanced_option
  {
    ...
    offs += fill_adv_frame_buf (adv_frame_buf + offs, prefix + 1, 2);	// address, 
    // 
    // 
    // 
    // control
    ...
    if (c != offs) {
      if (_debug)
	syslog (LOG_DEBUG,
		"Couldn't write the whole advanced option packet to the serial port for the virtual port %d. Wrote only %d bytes.",
		channel, c);
      return 0;
    }
  }
...
/*
 * Handles received data from ussp device. * * This function is derived
 * from a similar function in RFCOMM Implementation * with USSPs made by
 * Marcel Holtmann. * * PARAMS: * buf - buffer, which contains received
 * data * len - the length of the buffer * port - the number of ussp
 * device (logical channel), where data was * received * RETURNS: * the
 * number of remaining bytes in partial packet 
 */
int
ussp_recv_data (
  unsigned char *buf,
...
)
{
  ...
  for (top = op;
       /*
        * check for partial packet - first, make sure top->len is
        * actually in pkt_buf 
        */
       ((char *) top + sizeof (struct ussp_operation) <= ((char *) op) + n)
       && ((char *) top + sizeof (struct ussp_operation) + top->len <=
	   ((char *) op) + n);
       top =
       (struct ussp_operation *) (((char *) top) + top->len +
				  sizeof (struct ussp_operation))) {
    ...
    case USSP_WRITE:
      ...
      while ((written +=
	      write_frame (port + 1, top->data + written,
			   top->len - written, UIH)) != top->len
	     && i < WRITE_RETRIES) {
	i++;
      }
      if (i == WRITE_RETRIES) {
    ...
    case USSP_SET_TERMIOS:
      ...
      if ((tiosp->c_cflag & CBAUD) == B0
	  && (cstatus[(port + 1)].v24_signals & S_RTC) > 0) {
	// drop DTR
	PDEBUG ("Drop DTR.\n");
	msc[2] = 3 | ((63 & (port + 1)) << 2);
	...
	write_frame (0, msc, 4, UIH);
      ...
      else if ((tiosp->c_cflag & CBAUD) != B0
	       && (cstatus[(port + 1)].v24_signals & S_RTC) == 0) {
	// DTR up
	PDEBUG ("DTR up.\n");
	msc[2] |= ((63 & (port + 1)) << 2);
	...
	PDEBUG ("FC\n");
      ...
      cstatus[(port + 1)].v24_signals = msc[3];	// save
      // the
      // signals
      write_frame (0, msc, 4, UIH);
      break;
 ...
 * microseconds to wait for response (this is done 100 times) * RETURNS: * 
...
/*
 * Opens serial port, set's it to 57600bps 8N1 RTS/CTS mode. * * PARAMS: * 
 * dev - device name * RETURNS : * file descriptor or -1 on error 
 */
int
open_serialport (
  char *dev
...
)
{
  ...
  if (frame->data_length > 0) {
    if (_debug) {
      ...
      // fwrite(frame->data, sizeof(char), frame->data_length,
      // stdout);
      syslog (LOG_DEBUG, "\n");
    }
  }
...
)
{
  ...
  if (frame->data_length > 0) {
    type = frame->data[0];	// only a byte long types are
    // handled now
    // skip extra bytes
	...
	if (faultTolerant) {
	  ...
	  terminate = 1;
	  terminateCount = -1;	// don't need to
	  // close down
	  // channels
	}
	break;
	...
	// fwrite(frame->data + i, sizeof(char),
	// frame->data_length - i, stdout);
#endif
	break;
      case C_MSC:
...
)
{
  ...
  if (_debug)
    syslog (LOG_DEBUG, "is in %s\n", __FUNCTION__);
  while ((frame =
	  advanced_option ? gsm0710_buffer_adv_get_frame (buf) :
	  gsm0710_buffer_get_frame (buf))) {
    ++framesExtracted;
...
)
{
  ...
  if (!at_command (serial_fd, "AT\r\n", 10000)) {
    ...
    syslog (LOG_INFO,
	    "Modem does not respond to AT commands, trying close MUX mode");
#if 0				// multiplexer close down command doesn't
    // work with benqM22a module
...
#define PING_TEST_LEN 6
  ...
  programName = argv[0];
  if (_debug) {
    openlog (programName, LOG_NDELAY | LOG_PID | LOG_PERROR, LOG_LOCAL0);	// pode 
    // 
    // 
    // 
    // ir 
    ...
    openlog (programName, LOG_NDELAY | LOG_PID, LOG_LOCAL0);	// pode 
    // 
    // ir 
    // at� 
    // 7
  ...
  while (!terminate || terminateCount >= -1) {
	...
	if (FD_ISSET (ussp_fd[i], &rfds)) {
	  ...
	  if ((len =
	       read (ussp_fd[i], buf + remaining[i],
	  ...
	  /*
	   * copy remaining bytes from last packet into
	   * tmp 
	   */
	  if (remaining[i] > 0) {
    ...
    if (terminate) {
      // terminate command given. Close channels one by one and
      // finaly
      // close the mux mode

      if (terminateCount > 0) {
	syslog (LOG_INFO,
		"Closing down the logical channel %d.\n", terminateCount);
...
//              }
	...
	// Nothing has been received for a while -> test the 
	// 
	// modem
	if (_debug) {
